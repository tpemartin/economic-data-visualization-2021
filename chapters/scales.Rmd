...
```{r, echo=F}
samplePeriod <- dataSet2$x
start_label_tick <- lubridate::ymd("2013-01-01")
end_label_tick <- lubridate::ymd("2013-12-01") 
by="2 months"

ticks_labels <- list()
  # * always assign ticks to the beginning and the end of the sample period.  
ticks_labels$range <- range(samplePeriod)
  # * when too many ticks are there,  
  #   * try to label few of them, like labeling every 5 years, every 6 months, etc., and use different tick lengths; Or
ticks_labels$scale <- 
  scale_x_date(
    breaks=seq(from=start_label_tick, to=end_label_tick, by=by))
  #   * use period marking. .
ticks_labels$rug <-
  geom_rug(
    mapping=aes(
