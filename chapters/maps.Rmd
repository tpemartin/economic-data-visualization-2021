...
## Taiwan background map

```{r}
xfun::download_file(
  "https://www.dropbox.com/s/x5iinins676g53f/sf_taiwan.Rds?dl=1",
  output="data/sf_taiwan.Rds"
)
...
```{r , eval=T, echo=F}
teachDS::img_centering("img/gg_taiwan1_geom_sf.png", "100%")
```
...
```{r , eval=T, echo=F}
teachDS::img_centering("img/gg_taiwan2_geom_sf.png", "100%")
```

### stamenmap as background
...
# toner-lite
ggmap::get_stamenmap(bbox, maptype="toner-lite", zoom=9) -> taiwan_tonerlite
background_tw_tonerlite <- ggmap::ggmap(taiwan_tonerlite)
background_tw_tonerlite

# terrain
ggmap::get_stamenmap(bbox, maptype="terrain", zoom=9, language="zh-TW") -> taiwan_terrain
background_tw_terrain <- ggmap::ggmap(taiwan_terrain,
  darken=c("0.3", "white"))
background_tw_terrain
```

```{r}
ggplot(faithfuld, aes(waiting, eruptions)) +
 geom_raster(aes(fill = density), alpha=0.3) -> .gg
.gg |> View()
```
...
gg_drug <- list()
drug |> 
  dplyr::filter(發生西元年=="2018") -> gg_drug$data
library(dplyr)
gg_drug$data |>
  group_by(發生地點) |>
  summarise(
...
```{r}
sf_taiwan$區鎮_simplified_patched |> View()
sf_taiwan$區鎮_simplified_patched |> 
  sf_taiwan$crop_mainland()  -> .df
.df |> 
  mutate(
    location=paste0(
      stringr::str_replace(is_in.city, "臺", "台"), name)
  ) -> .df
.df |> #names()
  dplyr::left_join(
    gg_drug$data,
...
```{r}
gg_drug$map$background <- function(){
  sf_taiwan$區鎮_simplified_patched |> 
  sf_taiwan$crop_mainland() |> 
  mp$sf$make_background_map()
}
gg_drug$map$background()
```

```{r}
gg_drug$map$choropleth0 <- function(){
  gg_drug$map$background() +
    geom_sf(
      data=gg_drug$sf,
      mapping=aes(
...
gg_drug$map$choropleth1 <- function(){
  gg_drug$map$background() +
    geom_sf(
      data=gg_drug$sf,
      mapping=aes(
...
```{r}
gg_drug$map$choropleth1() +
  colorspace::scale_fill_discrete_sequential(
    palette="drug", 
    na.value="#dbd7a8"
  ) + 
  theme_void() -> gg_drug$map$choropleth_final

gg_drug$map$choropleth_final
```
...
<div class="alert alert-info">
If you want to make the entire Taiwan island's choropleth map down to "縣市" or "鄉鎮區", you can also do:
```{r}
ggplot()+
...
```{r}
get_stamenmap2 <- function(...){
  ggmap::get_stamenmap(...) -> map_stamen
  ggmap_bbox(map_stamen)
}
```
...
# Use the function:
taiwan_terrain2 <- ggmap_bbox(taiwan_terrain)
```


```{r}

ggmap(taiwan_terrain2) + 
...
```{r}
plotly::ggplotly(drug_tonerlite)
```
...
```{r}
plotly::ggplotly(drug_terrain)
```

<div class="alert alert-warning">
The overlaid map is a little off compared to the background map, since the sf data has been simplified to save the computation cost. Nonetheless, it is good for data mining purpose. 
</div>
