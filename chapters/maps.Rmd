...
A function that can generate color as a mixture of the pigments specified.
...
myPalette1(c(0, 0.5, 1)) |> scales::show_col()
```

Return a palette function based on you shiny interaction.
```{r}
myPalette2 <-
  colorspace::choose_palette(gui="shiny") 
myPalette2(n=3) # generate 3 equal distance color mixed
```

  * myPalette1 takes a numeric vector (element values all between 0 and 1) and return a vector of color codes.
  
  * myPalette2 takes an positive integer and return that number amount of color codes as a vector. 
  
  * With right pigments choice, `myPalette2(n=3) == myPalette1(c(0,0.5,1))`
...
```{r se_data2}
se_data2 <- se_data
se_data2$women_in_parliament |> IQR(na.rm=T)
se_data2$women_in_parliament |> quantile(c(0.25, 0.5, 0.75,1), na.rm = T) |> 
  round() -> cutpoints
# cut into 4 ordered levels
se_data2$women_in_parliament |>
  cut(c(0, cutpoints), ordered_result = T) -> .fct
.fct |> class()
# rename levels for legend labels
levels(.fct) <- c("0-15%","15-23%","23-31%","31-61%")
...
choropleth1() +
  scale_fill_manual(
    ...
    values = c("#919191", myPalette1(c(0, 0.33, 0.66, 1)))
  )+
  guides(
    fill=guide_legend(
      reverse = T
...
```
  ...
  * The chosen palette function will be `pal`.
...
```
  ...
  * The chosen palette function will be `mp$palette`

The chosen palette function can generate interpolate as many colors on the palette as you want

```{r}
mp$choose_palette |> args()
```


```{r}
pal(n=6) |> scales::show_col()
mp$palettte(n=7) |> scales::show_col()
mp$palettte(n=9) |> scales::show_col()
mp$palettte(register="blue_red")
```

Diverging color 

When dealing with diverging color it is better to rescale your color mapping source data to [-1, 1] based on what you really want. By default,
...
ntpu <- ggmap::get_stamenmap(
  bbox = c(left = 121.3636, bottom = 24.9379, right = 121.3866, top = 24.9520),
  zoom=16,
  maptype = "toner-lite"
)
ggmap(ntpu)
